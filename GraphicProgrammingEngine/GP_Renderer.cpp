...
void GP_Renderer::Init(HWND _hwnd, INT _width, INT _height, bool _isWindowed)
{
	...
	CheckFailed(hr, "CE_Renderer", 34, "Fail to Create the Device and Swap Chand");

	ID3D11Texture2D* backBuffer = nullptr;
	hr = d3dSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&backBuffer));
	CheckFailed(hr, "CE_Renderer", 38, "Fail to create the Backbuffer");

	hr = d3dDevice->CreateRenderTargetView(backBuffer, nullptr, &renderTargetView);
	CheckFailed(hr, "CE_Renderer", 41, "Fail to create the TargetView");
	...
	CheckFailed(hr, "CE_Renderer", 60, "Create Stencil Texture failed");

	hr = d3dDevice->CreateDepthStencilView(depthStencilBuffer, NULL, &depthStencilView);
	CheckFailed(hr, "CE_Renderer", 63, "Create Stencil View failed");
	...
	CheckFailed(hr, "CE_Renderer", 72, "Create Depth Stencil State failed");
	...
	CheckFailed(hr, "CE_Renderer", 81, "Create Rasterizer State failed");
	...
	CheckFailed(hr, "CE_Renderer", 96, "Create Blend State failed");
...
void GP_Renderer::BeginScene(FLOAT _red, FLOAT _green, FLOAT _blue)
{
...
void GP_Renderer::EndScene()
{
	d3dSwapChain->Present(0, 0);
}

void GP_Renderer::DeInit()
{
