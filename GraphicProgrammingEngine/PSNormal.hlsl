...
float3 CalculateDirectionalLight(DIR_LIGHT _dirLight, float3 _normal, float3 _viewDir)
{
    ...
    float diff = dot(_normal, lightDirection) * 0.75f; //_dirLight.m_intensity;
    float3 halfDirection = normalize(-lightDirection - _viewDir);
    float spec = pow(saturate(dot(-halfDirection, _normal)), 32); // 8 = spec strength
...
float4 main(PS_INPUT _input) : SV_TARGET
{
    ...
    normal += normalTexture.Sample(samplerState, _input.uv);
    normal = (normal * 2) - 1;
    //normal = (normal.x * _input.normal + normal.y * _input.normal + normal.z * _input.normal);
    normal = normalize(normal);
    //return float4(normal, 1);
    
    float3 viewDirection = normalize(_input.camPosition - _input.worldPosition);
    float3 result = CalculateDirectionalLight(dirLight, normal, viewDirection);
